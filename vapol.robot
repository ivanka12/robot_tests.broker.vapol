...
Підготувати клієнт для користувача
  ...
  Run Keyword If   '${username}' != 'vapol_viewer'   Login   ${username}
...
Створити тендер
  ...
  ...      ${ARGUMENTS[1]} ==  tender_data
    ...
    ${value_valueaddedtaxincluded}=          convert_vapol_string      ${value_valueaddedtaxincluded}
    ...
    ${auctionperiod_startdate}=        vapol_convertdate   ${auctionperiod_startdate}
...
Додати предмети
    ...
    \   vapol.Додати предмет   ${items[${index}]}     ${index}
...
Завантажити протокол аукціону
    ...
    vapol.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
    :FOR    ${i}    IN RANGE    1   5
...
Задати запитання на тендер
  ...
  vapol.Задати питання  ${username}  ${tender_uaid}  ${question}
...
Оновити сторінку з тендером
    ...
    Go to   ${USERS.users['${ARGUMENTS[0]}'].homepage}
    vapol.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[1]}
...
Отримати інформацію про awards[0].status
    ...
    ${return_value}=   convert_vapol_string     ${return_value}
    [Return]  ${return_value}
...
Внести зміни в тендер
  ...
  ...  ELSE IF  '${field_name}' == 'dgfDecisionDate'  vapol_convertdate  ${field_value}
...
Отримати інформацію про tenderPeriod.startDate
  ...
  ${return_value}=   convert_vapol_date_to_iso_format   ${return_value}
...
Отримати інформацію із документа
  ...
  ${tender}=  vapol.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
...
Отримати документ
    ...
    vapol_download_file   ${url}  ${file_name}  ${OUTPUT_DIR}
    [return]  ${file_name}
...
Подати цінову пропозицію
    ...
    Run Keyword If    ${presentsum}     vapol.Вказати цінову пропозицію   ${bid}
...
Завантажити фінансову ліцензію
    [Arguments]  ${username}  ${tender_uaid}  ${filepath}
    vapol.Завантажити документ в ставку  ${username}  ${filepath}  ${tender_uaid}
...
Отримати інформацію із пропозиції
    ...
    ${bid}=   vapol.Отримати пропозицію  ${field}
